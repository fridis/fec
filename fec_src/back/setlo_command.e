...
feature { NONE }
	...
	const: INTEGER; -- the 10 bits (0..2^10-1)
...
feature { BASIC_BLOCK }
	...
	-- erzeugt den Current entsprechenden Maschinenbefehl.
		do
			if symbol /= 0 then
				mc.reloc_lo10(mc.pc,symbol,addend)
			end;
			mc.asm_ari_imm(op3_or,dst_reg,const,dst_reg);
...
--------------------------------------------------------------------------------
	...
	-- erzeugt den Current entsprechenden Maschinenbefehl.
		do
			write_string("%Tor      "); write_integer(const) write_string(","); write_integer(dst_reg);
			write_string("%N");
